...
interface Room { id: string; name: string; type: "general" | "class" | "dm"; description?: string; members: string[]; createdAt: string; }
interface Message { id: string; roomId: string; senderId: string; senderName: string; senderAvatar: string; content: string; type: "text" | "voice" | "system" | "file"; voiceData?: string; fileData?: string; fileName?: string; fileSize?: string; replyToId?: string; replyToMessage?: { senderName: string; content: string; }; reported: boolean; reportedBy: string[]; createdAt: string; senderRole?: "admin" | "user"; }
interface PasswordResetRequest { id: string; userId: string; username: string; fullName: string; requestedAt: string; status: "pending" | "resolved"; }
interface Call { id: string; callerId: string; callerName: string; callerAvatar: string; receiverId: string; receiverName: string; status: "ringing" | "connected" | "ended"; startedAt: string; }
...
async function checkContentInappropriate(text: string): Promise<{ inappropriate: boolean; reason?: string }> {
  ...
  for (const word of INAPPROPRIATE_WORDS) {
    if (normalized.includes(word)) return { inappropriate: true, reason: `Contient des mots inappropriés identifiés hors-ligne (${word})` };
  }
  if (ai) {
    try {
...
async function startServer() {
  ...
  app.post("/api/auth/register", async (req, res) => {
    ...
    database.messages.push({
      ...
      roomId: "general_room", senderId: "system", senderName: "🔔 Système", senderAvatar: "",
      content: `${newUser.username} vient de rejoindre le groupe ! 🎉`,
      type: "system", reported: false, reportedBy: [], createdAt: new Date().toISOString()
    });
    saveDatabase();
    ...
    if (adminApprovalPending) {
      ...
      if (ambroiseUser) {
        ...
        database.messages.push({
          ...
          roomId: dmRoom.id, senderId: "system", senderName: "🚨 Système", senderAvatar: "",
          content: `⚠️ NOUVELLE DEMANDE CO-ADMINISTRATEUR :\n\nL'utilisateur @${newUser.username} s'est inscrit en fournissant le bon code.\nPour l'activer, rendez-vous dans le panneau d'administration.`,
          type: "system", reported: false, reportedBy: [], createdAt: new Date().toISOString()
        });
        saveDatabase();
      }
  ...
  app.post("/api/auth/login", (req, res) => {
    ...
    if (database.blacklistUsers && database.blacklistUsers.some(b => b.toLowerCase() === username.trim().toLowerCase())) {
      return res.status(403).json({ error: "🔒 Votre pseudo figure sur la liste noire." });
    }
    if (!user) return res.status(400).json({ error: "Ce pseudo n'existe pas." });
    ...
    if (user.isBanned) {
      if (user.banUntil) {
        ...
        if (until > new Date()) {
          const mLeft = Math.ceil((until.getTime() - Date.now()) / 60000);
          return res.status(403).json({ error: `🚫 Banni temporairement pour encore ${mLeft} min. Motif : ${user.banReason || ""}` });
        } else {
          user.isBanned = false; user.banUntil = undefined; user.banReason = undefined; saveDatabase();
        }
      } else {
        return res.status(403).json({ error: "🚫 Banni définitivement." });
      }
    }

    if (user.tempAdmin) {
      user.role = "user"; user.tempAdmin = false; saveDatabase();
    }
  ...
  app.post("/api/auth/forgot-password", (req, res) => {
    ...
    if (!username || !firstName || !lastName) return res.status(400).json({ error: "Tous les champs sont requis." });
    ...
    if (!user) return res.status(400).json({ error: "Aucun utilisateur trouvé." });

    database.passwordResets.push({ id: "req_" + Math.random().toString(36).substring(2, 9), userId: user.id, username: user.username, fullName: `${user.firstName} ${user.lastName}`, requestedAt: new Date().toISOString(), status: "pending" });
    saveDatabase();
    res.json({ success: true, message: "Demande envoyée à Ambroise." });
  });
  ...
  app.post("/api/users/update-pseudo", authenticateToken, async (req, res) => {
    ...
    if (user.role !== "admin") {
      ...
      if (diffMs / (1000 * 60 * 60) < 2) return res.status(400).json({ error: `Attendez 2 heures entre chaque changement.` });
    }

    if (database.users.find(u => u.username.toLowerCase() === cleanPseudo.toLowerCase() && u.id !== user.id)) return res.status(400).json({ error: "Ce pseudo est déjà pris." });
    if (cleanPseudo.endsWith("/admin") && user.role !== "admin") return res.status(400).json({ error: "Droits admin requis pour '/admin'." });

    const moderation = await checkContentInappropriate(cleanPseudo);
    if (moderation.inappropriate) return res.status(400).json({ error: `Pseudo inapproprié : ${moderation.reason}` });
  ...
  app.post("/api/rooms/dm", authenticateToken, (req, res) => {
    ...
    const user = (req as any).user as User;
    if (!partnerId) return res.status(400).json({ error: "ID requis." });
    const partner = database.users.find(u => u.id === partnerId);
    if (!partner) return res.status(404).json({ error: "Introuvable." });
    ...
    database.rooms.push(newRoom); saveDatabase();
    res.json({ id: newRoom.id });
  });

  app.post("/api/rooms", authenticateToken, (req, res) => {
    const user = (req as any).user as User;
    if (user.role !== "admin") return res.status(403).json({ error: "Admins uniquement." });
  ...
  app.put("/api/rooms/:roomId", authenticateToken, (req, res) => {
    ...
    if (!room) return res.status(404).json({ error: "Introuvable." });
    if (room.type === "dm") return res.status(400).json({ error: "Impossible." });
    if (!name || !name.trim()) return res.status(400).json({ error: "Nom requis." });
    room.name = name.trim();
    if (description !== undefined) room.description = description.trim();
    saveDatabase(); res.json(room);
  });
  ...
  app.delete("/api/rooms/:roomId", authenticateToken, (req, res) => {
    ...
    if (user.role !== "admin") return res.status(403).json({ error: "Admins uniquement." });
    const { roomId } = req.params;
    if (roomId === "general_room") return res.status(400).json({ error: "Impossible." });
    const roomIndex = database.rooms.findIndex(r => r.id === roomId);
    if (roomIndex === -1) return res.status(404).json({ error: "Introuvable." });
  ...
  app.get("/api/rooms/:roomId/messages", authenticateToken, (req, res) => {
    ...
    if (roomId.startsWith("dm_")) {
      const room = database.rooms.find(r => r.id === roomId);
      if (!room) return res.status(404).json({ error: "Introuvable." });
      if (!room.members.includes(user.id) && user.username !== "Ambroise/admin") return res.status(403).json({ error: "Non autorisé." });
    }
    ...
    const formattedList = list.map(m => {
      ...
      return { ...m, isBlocked: isSenderBlocked, content: isSenderBlocked ? "🚫 Message masqué" : m.content };
    });
  ...
  app.post("/api/rooms/:roomId/messages", authenticateToken, async (req, res) => {
    ...
    if (user.isMuted) {
      if (user.muteUntil) {
        if (new Date(user.muteUntil) > new Date()) return res.status(403).json({ error: `🤐 Mute actif.` });
        else { user.isMuted = false; user.muteUntil = undefined; user.muteReason = undefined; saveDatabase(); }
      } else return res.status(403).json({ error: "🤐 Mute définitif." });
    }

    if (!content && !voiceData && !fileData) return res.status(400).json({ error: "Vide." });
    const rawIp = (req.headers["x-forwarded-for"] as string || req.socket.remoteAddress || "").split(",")[0].trim();
    ...
    if (trimContent.startsWith("/")) {
      ...
      if (commandName === "/help") {
        const helpMsg: Message = { id: "msg_sys_help_" + Math.random().toString(36).substring(2, 9), roomId, senderId: "system", senderName: "🔔 Assistant", senderAvatar: "", content: `Commandes :\n/help\n/status\n/report\n/block <pseudo>\n/unblock <pseudo>\n/mute <pseudo> <min> <raison>\n/unmute <pseudo>\n/ban <pseudo> <min> <raison>\n/unban <pseudo>\n/promote <pseudo>\n/clear @a\n/clear @<pseudo>`, type: "system", reported: false, reportedBy: [], createdAt: new Date().toISOString() };
        database.messages.push(helpMsg); saveDatabase(); return res.json(helpMsg);
      }

      if (commandName === "/status") {
        const systemMsg: Message = { id: "msg_sys_status_" + Math.random().toString(36).substring(2, 9), roomId, senderId: "system", senderName: "🔔 Assistant", senderAvatar: "", content: `IP: ${rawIp}\nRôle: ${user.role}`, type: "system", reported: false, reportedBy: [], createdAt: new Date().toISOString() };
        database.messages.push(systemMsg); saveDatabase(); return res.json(systemMsg);
      }

      if (commandName === "/report") {
        const systemMsg: Message = { id: "msg_sys_rep_" + Math.random().toString(36).substring(2, 9), roomId, senderId: "system", senderName: "🔔 Assistant", senderAvatar: "", content: `Cliquez sur le triangle rouge.`, type: "system", reported: false, reportedBy: [], createdAt: new Date().toISOString() };
        database.messages.push(systemMsg); saveDatabase(); return res.json(systemMsg);
      }
      ...
      if (commandName === "/block" || commandName === "/unblock") {
        ...
        if (!targetUser) return res.status(400).json({ error: "Introuvable." });
        ...
        if (isBlocking) { if (targetUser.id === user.id) return res.status(400).json({ error: "Erreur." }); blockedSet.add(targetUser.id); } else blockedSet.delete(targetUser.id);
        database.users[uIndex].blockedUsers = Array.from(blockedSet);
        saveDatabase();
        const systemMsg: Message = { id: "msg_sys_blok_" + Math.random().toString(36).substring(2, 9), roomId, senderId: "system", senderName: "🔔", senderAvatar: "", content: isBlocking ? `🚫 Bloqué.` : `✅ Débloqué.`, type: "system", reported: false, reportedBy: [], createdAt: new Date().toISOString() };
        database.messages.push(systemMsg); saveDatabase(); return res.json(systemMsg);
      }

      if (commandName === "/clear") {
        if (user.username !== "Ambroise/admin") return res.status(403).json({ error: "Réservé Ambroise." });
        const choice = parts.slice(1).join(" ").trim();
        if (choice === "@a") {
          database.users = []; database.passwordResets = []; database.calls = []; database.rooms = database.rooms.map(r => ({ ...r, members: [] }));
          database.messages = [{ id: "msg_sys_reset", roomId: "general_room", senderId: "system", senderName: "🚨 Système", senderAvatar: "", content: "⚠️ RESET COMPLET.", type: "system", reported: false, reportedBy: [], createdAt: new Date().toISOString() }];
          database.systemResetNotice = true; saveDatabase();
          return res.json({ id: "clear_triggered", roomId, senderId: "system", senderName: "🚨", senderAvatar: "", content: "Purge totale.", type: "system", reported: false, reportedBy: [], createdAt: new Date().toISOString() });
        } else {
          ...
          if (targetIndex === -1) return res.status(404).json({ error: "Introuvable." });
          if (database.users[targetIndex].username === "Ambroise/admin") return res.status(400).json({ error: "Impossible." });
          database.users.splice(targetIndex, 1);
          const systemMsg: Message = { id: "msg_sys_clear_usr_" + Math.random().toString(36).substring(2, 9), roomId, senderId: "system", senderName: "🚨", senderAvatar: "", content: `🚫 Compte @${targetUsername} supprimé.`, type: "system", reported: false, reportedBy: [], createdAt: new Date().toISOString() };
          database.messages.push(systemMsg); saveDatabase(); return res.json(systemMsg);
        }
      }
      ...
      if (commandName === "/mute") {
        ...
        const durationMin = parseInt(parts[2], 10);
        ...
        if (targetUser.username === "Ambroise/admin") return res.status(400).json({ error: "Impossible." });
        targetUser.isMuted = true;
        const untilDate = new Date(); untilDate.setMinutes(untilDate.getMinutes() + durationMin);
        targetUser.muteUntil = untilDate.toISOString(); targetUser.muteReason = parts.slice(3).join(" ").trim();
        saveDatabase();
        const systemMsg: Message = { id: "m_" + Math.random(), roomId, senderId: "system", senderName: "🛡️", senderAvatar: "", content: `🤐 @${targetUser.username} rendu muet.`, type: "system", reported: false, reportedBy: [], createdAt: new Date().toISOString() };
        database.messages.push(systemMsg); saveDatabase(); return res.json(systemMsg);
      }
      ...
      if (commandName === "/unmute") {
        ...
        const targetUser = database.users.find(u => u.username.toLowerCase() === targetUsername.toLowerCase());
        if (!targetUser) return res.status(404).json({ error: "Introuvable." });
        targetUser.isMuted = false; targetUser.muteUntil = undefined; targetUser.muteReason = undefined; saveDatabase();
        const systemMsg: Message = { id: "m_" + Math.random(), roomId, senderId: "system", senderName: "🛡️", senderAvatar: "", content: `🔊 @${targetUser.username} n'est plus muet.`, type: "system", reported: false, reportedBy: [], createdAt: new Date().toISOString() };
        database.messages.push(systemMsg); saveDatabase(); return res.json(systemMsg);
      }
      ...
      if (commandName === "/ban") {
        ...
        const untilDate = new Date(); untilDate.setMinutes(untilDate.getMinutes() + durationMin);
        targetUser.banUntil = untilDate.toISOString(); targetUser.banReason = parts.slice(3).join(" ").trim();
        saveDatabase();
        const systemMsg: Message = { id: "m_" + Math.random(), roomId, senderId: "system", senderName: "🛡️", senderAvatar: "", content: `🚨 @${targetUser.username} banni.`, type: "system", reported: false, reportedBy: [], createdAt: new Date().toISOString() };
        database.messages.push(systemMsg); saveDatabase(); return res.json(systemMsg);
      }
      ...
      if (commandName === "/unban") {
        ...
        const targetUser = database.users.find(u => u.username.toLowerCase() === targetUsername.toLowerCase());
        if (!targetUser) return res.status(404).json({ error: "Introuvable." });
        targetUser.isBanned = false; targetUser.banUntil = undefined; targetUser.banReason = undefined; saveDatabase();
        const systemMsg: Message = { id: "m_" + Math.random(), roomId, senderId: "system", senderName: "🛡️", senderAvatar: "", content: `✅ @${targetUser.username} débanni.`, type: "system", reported: false, reportedBy: [], createdAt: new Date().toISOString() };
        database.messages.push(systemMsg); saveDatabase(); return res.json(systemMsg);
      }

      if (commandName === "/promote") {
        if (user.username !== "Ambroise/admin") return res.status(403).json({ error: "Ambroise uniquement." });
        const targetUsername = (parts[1] || "").replace("@", "");
        const targetUser = database.users.find(u => u.username.toLowerCase() === targetUsername.toLowerCase());
        if (!targetUser) return res.status(404).json({ error: "Introuvable." });
        targetUser.role = "admin"; targetUser.tempAdmin = true; saveDatabase();
        const systemMsg: Message = { id: "m_" + Math.random(), roomId, senderId: "system", senderName: "🚨", senderAvatar: "", content: `👑 @${targetUser.username} promu Admin temporaire.`, type: "system", reported: false, reportedBy: [], createdAt: new Date().toISOString() };
        database.messages.push(systemMsg); saveDatabase(); return res.json(systemMsg);
      }
    ...
    if (type === "text" || (content && type !== "voice")) {
      ...
      if (check.inappropriate) {
        const botMessage: Message = { id: "m_" + Math.random(), roomId, senderId: "bot_modo", senderName: "🤖 Bot", senderAvatar: "", content: `⚠️ Message bloqué.`, type: "text", reported: false, reportedBy: [], createdAt: new Date().toISOString() };
        database.messages.push(botMessage); saveDatabase(); return res.status(400).json({ error: "🚫 Bloqué." });
      }
    }
    ...
    if (replyToId) {
      ...
      if (targetMsg) replyToMessageCtx = { senderName: targetMsg.senderName, content: targetMsg.content };
    }

    const newMsg: Message = { id: "m_" + Math.random(), roomId, senderId: user.id, senderName: user.username, senderAvatar: user.avatarUrl, content: content || "", type: type || "text", voiceData, fileData, fileName, fileSize, replyToId, replyToMessage: replyToMessageCtx, reported: false, reportedBy: [], createdAt: new Date().toISOString() };
    database.messages.push(newMsg); saveDatabase(); res.json(newMsg);
  });

  app.post("/api/messages/:messageId/report", authenticateToken, (req, res) => {
    const { messageId } = req.params; const user = (req as any).user as User;
  ...
  app.post("/api/messages/:messageId/delete", authenticateToken, (req, res) => {
    const { messageId } = req.params; const user = (req as any).user as User;
    if (user.role !== "admin") return res.status(403).json({ error: "Admins." });
    const index = database.messages.findIndex(m => m.id === messageId);
    if (index === -1) return res.status(404).json({ error: "Introuvable." });
    database.messages[index] = { ...database.messages[index], content: "🚫 Supprimé.", type: "system", voiceData: undefined, replyToId: undefined, replyToMessage: undefined };
    saveDatabase(); res.json({ success: true });
  });

  app.post("/api/users/block", authenticateToken, (req, res) => {
    const { targetUserId } = req.body; const user = (req as any).user as User;
    if (targetUserId === user.id) return res.status(400).json({ error: "Erreur." });
    const uIndex = database.users.findIndex(u => u.id === user.id);
    if (!database.users[uIndex].blockedUsers) database.users[uIndex].blockedUsers = [];
    const blockedSet = new Set(database.users[uIndex].blockedUsers);
    if (blockedSet.has(targetUserId)) blockedSet.delete(targetUserId); else blockedSet.add(targetUserId);
    database.users[uIndex].blockedUsers = Array.from(blockedSet);
    saveDatabase(); res.json({ success: true, blockedUsers: database.users[uIndex].blockedUsers });
  });
  ...
  app.post("/api/calls/initiate", authenticateToken, (req, res) => {
    const { receiverId } = req.body; const user = (req as any).user as User;
    const receiver = database.users.find(u => u.id === receiverId);
    if (!receiver) return res.status(444).json({ error: "Introuvable." });
    ...
    const newCall: Call = { id: "c_" + Math.random(), callerId: user.id, callerName: user.username, callerAvatar: user.avatarUrl, receiverId: receiver.id, receiverName: receiver.username, status: "ringing", startedAt: new Date().toISOString() };
    database.calls.push(newCall); saveDatabase(); res.json({ call: newCall });
  });

  app.post("/api/calls/:callId/respond", authenticateToken, (req, res) => {
    const callIndex = database.calls.findIndex(c => c.id === req.params.callId);
    if (callIndex === -1) return res.status(404).json({ error: "Introuvable." });
    database.calls[callIndex].status = req.body.action === "accept" ? "connected" : "ended";
    saveDatabase(); res.json({ call: database.calls[callIndex] });
  });
  ...
  app.get("/api/admin/requests", authenticateToken, (req, res) => { res.json({ requests: database.passwordResets }); });
  
  app.post("/api/admin/requests/:reqId/resolve", authenticateToken, (req, res) => {
    const requestIndex = database.passwordResets.findIndex(r => r.id === req.params.reqId);
    if (requestIndex === -1) return res.status(404).json({ error: "Introuvable." });
    database.passwordResets[requestIndex].status = "resolved";
    const uIndex = database.users.findIndex(u => u.id === database.passwordResets[requestIndex].userId);
    if (uIndex !== -1) database.users[uIndex].forcePasswordReset = true;
    saveDatabase(); res.json({ success: true });
  });

  app.get("/api/admin/passwords", authenticateToken, (req, res) => { res.json({ users: database.users }); });
  app.get("/api/admin/reports", authenticateToken, (req, res) => { res.json({ reports: database.messages.filter(m => m.reported) }); });

  app.post("/api/admin/ban", authenticateToken, (req, res) => {
    const target = database.users.find(u => u.id === req.body.targetUserId);
    if (!target) return res.status(404).json({ error: "Introuvable." });
    target.isBanned = !target.isBanned; saveDatabase(); res.json({ success: true, isBanned: target.isBanned });
  });

  app.post("/api/admin/approve-admin", authenticateToken, (req, res) => {
    const uIndex = database.users.findIndex(u => u.id === req.body.targetUserId);
    if (uIndex === -1) return res.status(404).json({ error: "Introuvable." });
    database.users[uIndex].role = "admin"; database.users[uIndex].adminApprovalPending = false; saveDatabase(); res.json({ success: true });
  });

  app.get("/api/admin/blacklist", authenticateToken, (req, res) => { res.json({ blacklist: database.blacklistUsers || [] }); });
  app.post("/api/admin/blacklist", authenticateToken, (req, res) => {
    if (!database.blacklistUsers) database.blacklistUsers = [];
    database.blacklistUsers.push(req.body.username.trim()); saveDatabase(); res.json({ success: true });
  });
  app.delete("/api/admin/blacklist/:username", authenticateToken, (req, res) => {
    if (database.blacklistUsers) database.blacklistUsers = database.blacklistUsers.filter(b => b.toLowerCase() !== req.params.username.trim().toLowerCase());
    saveDatabase(); res.json({ success: true });
  });
  
  app.post("/api/admin/reset-all", authenticateToken, (req, res) => {
    database.users = []; database.passwordResets = []; database.calls = []; database.rooms = database.rooms.map(r => ({ ...r, members: [] }));
    database.systemResetNotice = true; saveDatabase(); res.json({ success: true });
  });
